...
public static class ArgumentValidation
{
    ...
    public static void ValidateAcountCreation([NotNull] string? name, [NotNull] string? surname)
    {
        ...
        static void ValidateName([NotNull] string? name, string fieldName)
        {
            if (string.IsNullOrWhiteSpace(name))
                throw new BankException($"{fieldName} cannot be empty");
            if (name.Length > 32)
                throw new BankException($"{fieldName} cannot be longer than 32 characters");
        }
    }
    ...
    /// <exception cref="BankException"><paramref name="amount"/> is negative.</exception>
    ...
    /// <exception cref="BankException">Either <paramref name="originAccountId"/> and
    ...
    public static void ValidateTransfer(AccountId originAccountId, AccountId destinationAccountId, decimal amount)
    {
        if (originAccountId == destinationAccountId)
            throw new BankException("Origin and destination account cannot be the same");
